...
/*
  prop p : Forall timestamp . Forall user . Forall db . Forall p . Forall d . delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'') 
*/

class Formula_p(monitor: Monitor) extends Formula(monitor) {
          
  val var_timestamp :: var_user :: var_db :: var_p :: var_d :: Nil = declareVariables(("timestamp",false), ("user",false), ("db",false), ("p",false), ("d",false))

  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(7) = build("delete")(V("timestamp"),V("user"),V("db"),V("p"),V("d"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(8) = relationToConstant("db",EQOP,"db2").or(pre(8))
      now(6) = now(7).and(now(8))
      now(9) = relationToConstant("user",EQOP,"script2").or(pre(9))
      now(5) = now(6).not().or(now(9))
      now(4) = now(5).forAll(var_d.quantvar)
      now(3) = now(4).forAll(var_p.quantvar)
      now(2) = now(3).forAll(var_db.quantvar)
      now(1) = now(2).forAll(var_user.quantvar)
      now(0) = now(1).forAll(var_timestamp.quantvar)
  ...
  txt = Array(
    "Forall timestamp . Forall user . Forall db . Forall p . Forall d . delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'')",
      "Forall user . Forall db . Forall p . Forall d . delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'')",
      "Forall db . Forall p . Forall d . delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'')",
      "Forall p . Forall d . delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'')",
      "Forall d . delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'')",
      "delete(timestamp,user,db,p,d) & (db = ''db2'') -> (user = ''script2'')",
      "delete(timestamp,user,db,p,d) & (db = ''db2'')",
      "delete(timestamp,user,db,p,d)",
      "db = ''db2''",
      "user = ''script2''"
  )
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("commit","script_end","delete","update","script_start","script_svn","script_md5","select","insert")
